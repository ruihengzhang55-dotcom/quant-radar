...
LANG = {
    "zh": {
        ...
        "paywall_title": "🔒 该功能需要订阅",
        "paywall_sub": "您的7天免费试用已结束",
        "paywall_free": "免费版可用：",
        "paywall_free_items": "• 每日前5条信号预览\n• 查看信号类型",
        "paywall_paid": "订阅后解锁：",
        "paywall_paid_items": "• 全市场90+只股票扫描\n• 完整量化诊断\n• 历史回测数据\n• CSV导出",
        "paywall_price": "订阅价格：¥29/月 或 ¥199/年",
        "paywall_contact": "👉 联系微信：your_wechat 完成订阅",
        ...
        "market_us_large": "🇺🇸 美股大盘", "market_us_small": "🚀 美股小盘",
        "market_a": "🇨🇳 A股精选", "market_mix": "🌏 美股+A股",
        "market_custom": "🔧 自定义",
        "bt_header": "🔬 单股历史回测",
...
本工具仅供策略研究，不构成投资建议。"""
    ...
    "en": {
        ...
        "paywall_paid_items": "• 90+ tickers scanned\n• Full AI diagnosis\n• Backtest data\n• CSV export",
        ...
        "market_us_large": "🇺🇸 US Large Cap", "market_us_small": "🚀 US Small Cap",
...
US_LARGE_CAP = ["AAPL","MSFT","NVDA","AMZN","GOOGL","META","TSLA","AMD","INTC","QCOM",
    "NFLX","CRM","ORCL","ADBE","MU","AVGO","ARM","SMCI","PLTR","SNOW",
    "UBER","LYFT","RBLX","HOOD","COIN","MSTR","SOFI","UPST","AFRM","SQ",
    "SHOP","MELI","SE","BABA","JD","NIO","XPEV","LI","PDD","BIDU"]
...
if not is_logged_in():
    ...
    for col,icon,title,desc in [
        (c1,"📡","全市场扫描" if lang_choice=="中文" else "Full Market Scan","覆盖美股+A股90+只" if lang_choice=="中文" else "90+ US & A-share tickers"),
            ...
            if not results:
                ...
                if not filtered:
                    ...
                    def cchg(v):
                        ...
                        return ""
                    st.dataframe(df.style.applymap(csig,subset=[L["col_status"]]).applymap(cchg,subset=[L["col_chg"]]), use_container_width=True, height=480)
                    if is_full:
    ...
    if status["access"] == "locked":
        ...
        if run_bt and ticker_input.strip():
            ...
            if not bt_res or not bt_res["signals"]:
                ...
                st.dataframe(df_bt.style.applymap(cret,subset=[L["bt_col_r5"],L["bt_col_r7"]]), use_container_width=True)
